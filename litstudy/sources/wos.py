...
from ..types import Author, Document, DocumentSet, DocumentIdentifier, Affiliation
...
class WosAuthor(Author):
    ...
    def affiliations(self):
        ...
        return [WosAffiliation(self._affiliation)]


class WosAffiliation(Affiliation):
    def __init__(self, name):
...
class WosDocument(Document):
    ...
    @property
    def authors(self):
        authors = self._field("authors").split("; ")
        affs = self._field("affiliations").split("; ")
        ...
        return [WosAuthor(a, b) for a, b in zip(authors, affs)]
    ...
    #def authors(self):
        ...
        #for delim in [";", "|", " and ", ","]:
                ...
                #return [WosAuthor(name) for name in names]

        ##Just one author?
        #return [WosAuthor(text)]
...
def load_wos_csv(
    ...
    docs = [WosDocument(record, fields) for record in records]
